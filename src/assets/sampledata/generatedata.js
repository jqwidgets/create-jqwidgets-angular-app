...
export function generateordersdata(rowscount) {
    ...
    for (var i = 0; i < rowscount; i++) {
        ...
        for (var j = 0; j < subRowsCount; j++) {
            var subRow = {};
            var productindex_1 = Math.floor(Math.random() * productNames.length);
            var price_1 = parseFloat(priceValues[productindex_1]);
            var quantity_1 = 1;
            subRow['name'] = productNames[productindex_1];
            subRow['id'] = '' + i + '.' + (1 + j);
            subRow['parentid'] = i;
            subRow['price'] = price_1;
            subRow['quantity'] = 1;
            var date_1 = new Date();
            date_1.setFullYear(2016, month, day);
            date_1.setHours(Math.floor(Math.random() * 23), Math.floor(Math.random() * 59), 0, 0);
            subRow['date'] = date_1;
            row['firstname'] = firstNames[Math.floor(Math.random() * firstNames.length)];
            row['lastname'] = lastNames[Math.floor(Math.random() * lastNames.length)];
            subRow['customer'] = row['firstname'] + ' ' + row['lastname'];
            t += quantity_1 * price_1;
            data.push(subRow);
            q += quantity_1;
        }
        row['price'] = t;
...
export function generateData(rowscount) {
